...
import { Users, Minus, Maximize2 } from "lucide-react"; // Import Icons
...
export function UserSwitcher() {
    ...
    if (isCollapsed) {
        return (
            <Button
                variant="outline"
                className="fixed bottom-4 right-4 z-50 shadow-xl bg-white/90 backdrop-blur border-indigo-200 text-indigo-700 hover:bg-indigo-50"
            ...
            <CardHeader className="py-3 px-4 bg-indigo-50 border-b border-indigo-100 rounded-t-lg">
                <div className="flex items-center justify-between">
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 hover:bg-indigo-200/50 text-indigo-500"
                        ...
                        >
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0
                                ${user.role === 'manager' ? 'bg-purple-600' :
                                    user.role === 'director' ? 'bg-orange-600' :
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-500 capitalize">{user.role}</p> {/* ID Removed */}
                            </div>
